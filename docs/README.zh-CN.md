...
<p align="center">
  <strong>跨设备会话式同步</strong> — 把文本、剪贴板、图片、视频和大文件发到你的每台设备。局域网优先，跨网可用 S3 兼容存储后备。
</p>
...
本仓库（**`ultrasend`**）是 **ShrimpSend / 虾传** 的开源源码。虾传是面向个人多设备的消息与文件中转：基于 Centrifugo 多端实时同步，支持文本与文件；文件可走 S3 广域网或局域网直连（含反向连接）。它不是网盘，也不是「上传生成链接再转发」的工具。

## 为何选择虾传

- **设备会话** — 按目标设备组织发送记录，文本、截图、安装包、视频可留在同一会话里。
- **局域网 / WebRTC 优先** — 同网尽量直连；NAT、防火墙或单向连通时自动切换路径。
- **实时同步** — 登录设备订阅频道 `user#<userId>`，消息即时推送。
- **灵活的文件投递** — 全员 S3 预签名上传/下载，或指定单台设备走局域网直传/反向拉取（见 [shared/protocol.md](../shared/protocol.md)）。
...
<p align="center">
  <em>左：局域网发现设备 · 中：设备会话（HTTP/WebRTC/S3）· 右：云端中继配置</em>
</p>
...
传输细节：局域网 HTTP 直推/反向拉取、跨网 S3 分片等见 [shared/protocol.md](../shared/protocol.md)。
